...
avlSearch(AvlTree t, tw_event *key)
{
    ...
    if (TW_STIME_CMP(key->recv_ts, t->key->recv_ts) == 0) {
        ...
        // if (key->sig.causal_ordering_value == t->key->sig.causal_ordering_value) {
        ...
        // }
        // else {
        //     return avlSearch(t->child[key->sig.causal_ordering_value > t->key->sig.causal_ordering_value], key);
        // }
#endif
    }
    else {
...
avlInsert(AvlTree *t, tw_event *key)
{
    ...
    if (TW_STIME_CMP(key->recv_ts, (*t)->key->recv_ts) == 0) {
        ...
        // }
        // else {
        //     avlInsert(&(*t)->child[key->sig.causal_ordering_value > (*t)->key->sig.causal_ordering_value], key);
        // }
#endif
    }
    else {
...
avlDelete(AvlTree *t, tw_event *key)
{
    ...
    if (TW_STIME_CMP(key->recv_ts, (*t)->key->recv_ts) == 0) {
#ifdef USE_RAND_TIEBREAKER
        // We have a timestamp tie, check the event tiebreakers
        // if (key->sig.causal_ordering_value == (*t)->key->sig.causal_ordering_value) {
        ...
        // }
        // else {
        //     target = avlDelete(&(*t)->child[key->sig.causal_ordering_value > (*t)->key->sig.causal_ordering_value], key);
        // }
#endif
    }
    else {
